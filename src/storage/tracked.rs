...
pub enum Change {
    /// `None`s are filtered out and the user should never need to deal with it.
    #[doc(hidden)]
    None,
    ...
    D: Deref<Target = MaskedStorage<T>>,
{
    ...
    ///
    /// let events: Vec<_> = w.read::<Comp>().change_events_tracked().collect();
    /// assert_eq!(events, vec![a.id(), Change::Inserted]);
    /// ```
...
{
    /// Returns an iterator for all change events.
