...
mod test {
    ...
    fn deferred_storage() {
        ...
        for mut comps in (&mut s1.defer_mut()).join() {
            let c1 = { comps.get_deferred().0 };

            let c2 = { comps.get_mut_deferred().0 };
    ...
    fn par_deferred_storage() {
        ...
        (&mut s1.par_defer_mut())
            .par_join()
            .for_each(|mut comps| {
                let (mut components2, mut components2_mut) =
                    (components2.lock().unwrap(), components2_mut.lock().unwrap());
                components2.push(comps.get_deferred().0);
                components2_mut.push(comps.get_mut_deferred().0);
            });
